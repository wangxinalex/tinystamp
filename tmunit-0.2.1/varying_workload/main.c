...
unsigned long agregCommits(bool *stop) {
	unsigned long sum = 0;
    unsigned ThreadNo;
//	sum= (long)(((ThLocalVarCollection*)(Statistics[ThreadNo]->locals)));
//	printf("Debug: ThreadNum=%u sum=%ld\n",ThreadNum,sum);
	for(ThreadNo=0; ThreadNo<ThreadNum; ThreadNo++) {
		if((ThLocalVarCollection*)(Statistics[ThreadNo]->locals))
			sum+=(((ThLocalVarCollection*)(Statistics[ThreadNo]->locals))->Statistics).CommitNum;
		else
			*stop=1;
	}
//	printf("Debug: Was here 54\n");
	return sum;
}
